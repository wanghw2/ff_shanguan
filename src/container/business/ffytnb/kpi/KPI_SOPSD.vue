        ...
        <el-tab-pane :label="menuArr[0]" class="pdtItem">
          <div class="block">
            <h2>YTNB CRAD默认天数
              <div class="form-box">
                <upload
                  ...
                  :logTableName="'YTNB_CRAD_Default_Day'"
                  :sheetsName="'YTNB_CRAD默认天数'"
                  :logObj="'gridcradday'">
                </upload>
              </div>              
            </h2>
            <div class="addgrid">
              <v-table
                :sheetsName="'YTNB_CRAD默认天数'"
                :tableName="'YTNB_CRAD_Default_Day'"
                :source="'FF_IDSS_Data_YT'"
                :fileName="'YTNB_CRAD默认天数'"
                ref="gridcradday">
              </v-table>
            </div>
          </div>
          ...
          <div class="block">
            <h2>YTNB FPSD手工订单
             <a href="javascript:;" class="fold" @click="toggleList1" v-text="toggle1 ? '折叠' : '展开'"></a>
              <div class="form-box">
                <upload
                  :sheetsName="'YTNB_FPSD手工订单'"
                  @on-success="updatefpsdsg"
                  @updateTimeChange="updatefpsdsg1"
                  :logTableName='"YTNB_FPSD_Manage"'
                  :logObj="'gridfpsdsg'">
                </upload>
              </div>
            </h2>
            <div class="addgrid addgrid1">
              <v-table
                :sheetsName="'YTNB_FPSD手工订单'"
                :tableName="'YTNB_FPSD_Manage'"
                :source="'FF_IDSS_Data_YT'"
                :fileName="'YTNB_FPSD手工订单'"
                ref="gridfpsdsg">
              </v-table>
            </div>
            <h2>YTNB FPSD手工订单 汇总
            ...
            <div class="addgrid addgrid2">
              <v-table
                :sheetsName="'YTNB_FPSD手工订单_汇总'"
                :tableName="'YTNB_FPSD_Manage_TTL'"
                :source="'FF_IDSS_Data_YT'"
                :fileName="'YTNB_FPSD手工订单_汇总'"
                ref="gridfpsdsgttl">
              </v-table>
            </div>
          </div>
        ...
        <el-tab-pane :label="menuArr[3]" class="pdtItem">
          <div class="block">
             ...
             <a :class="['small', 'emailBtn', {'disabled': disabledemail}]" @click="sendemail1" href="#">发送邮件</a>
              ...
              <div class="form-box">
                <updata
                  ...
                  :logTableName="'YTNB_KPI_CRAD_FPSD_SO_Detail'"
                  :proIndex="'202'">
                </updata>
              </div>
              <div class="form-box">
            ...
            <div class="addgrid addgrid3">
              <v-table
                :sheetsName="'FPSD分析'"
                :tableName="'YTNB_KPI_CRAD_FPSD_SO_Detail'"
                :source="'FF_IDSS_Data_YT'"
                :fileName="'FPSD分析'"
                ref="gridkpi">
              </v-table>
    ...
    download1 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 0,
          DOWNLOAD_BAT_PATH: 'YTNB_FPSD分析_张数_Download'
        },
        done (data) {
          let $a = $('<a></a>')
    ...
    download2 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 0,
          DOWNLOAD_BAT_PATH: 'YTNB_FPSD分析_数量_Download'
        },
        done (data) {
          let $a = $('<a></a>')
          $a.attr('href', data)[0].click()
    ...
    download3 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 0,
          DOWNLOAD_BAT_PATH: 'YTNB_CRAD分析_张数_Download'
        },
        done (data) {
          let $a = $('<a></a>')
          $a.attr('href', data)[0].click()
    ...
    download4 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 0,
          DOWNLOAD_BAT_PATH: 'YTNB_CRAD分析_数量_Download'
        },
        done (data) {
          let $a = $('<a></a>')
          $a.attr('href', data)[0].click()
