...
export default {
  data () {
    return {
      columns: [
          {title: 'Tablename Show', field: 'Tablename_Show', width: '120px'},
          {title: 'Index', field: 'Index', width: '120px'},
          {title: 'DBname', field: 'DBname', width: '120px'},
          {title: 'Tablename', field: 'Tablename', width: '120px'},
          {title: 'SheetName', field: 'SheetName', width: '120px'},
          {title: 'Select', field: 'Select', width: '120px'},
          {title: 'Where', field: 'Where', width: '120px'},
          {title: 'Order', field: 'Order', width: '120px'},
          {title: 'Colname Version', field: 'Colname_Version', width: '120px'},
          {title: 'Suffix', field: 'Suffix', width: '120px'},
          {title: 'Remark', field: 'Remark', width: '120px'},
          {title: 'Role_Show_Name', field: 'Role_Show_Name', width: '120px'}
      ],
      dataSource: [],
      fieldsList: [],
      pageSize: 18,
  ...
  methods: {
    ...
    initTab () { // 初始化表格
      ...
      $(this.$refs.table).kendoGrid({
        ...
        resizable: true,
        excel: {
          fileName: '254_下载配置参数' + '_' + vm.getDate1() + '.xlsx',
          allPages: true
        },
        pageable: {
          numeric: true,
        ...
        excelExport (e) {
          e.workbook.sheets[0].name = 'FF_下载配置参数'
        },
        columns: vm.columns,
        dataSource: {
          data: vm.dataSource,
    ...
    getDataSource () { // 获取表格数据
      ...
      return this.$ajax({
        ...
        method: 'get',
        params: {
          tableName: 'Table_Download_Excel_Para',
          source: 'FF_IDSS'
        },
        done (data) {
    ...
    uploadFile (e) {
      ...
      data.append('tableName', 'Table_Download_Excel_Para')
      let vm = this
      this.$ajax({
        url: '/uploadExcelParaAndloop',
        headers: {
