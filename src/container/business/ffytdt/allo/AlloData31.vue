        ...
        <el-tab-pane :label="menuArr[0]" class="alloItem">
          <div class="block">
            <h2>YTDT 31 比例类型
              <div class="form-box">
                <Upload
                  :sheetsName="'YTDT_比例类型_31'"
                  @on-success="updatefhlx31"
                  @updateTimeChange="updatefhlx311"
                  :logTableName="'YTDT_Alloc_RateType_31'"
                  :logObj="'ytdtfhlx31'" >
                </Upload>
              </div>
            </h2>
            <div class="addgrid addgrid1">
              <v-table
                :sheetsName="'YTDT_比例类型_31'"
                :tableName="'YTDT_Alloc_RateType_31'"
                :source="'FF_IDSS_Data_YT'"
                :fileName="'YTDT_比例类型_31'"
                ref="ytdtfhlx31">
              </v-table>
            </div>
          </div>  
        ...
        <el-tab-pane :label="menuArr[1]" class="alloItem">
          <div class="block">             
            <h2>YTDT 31 分货占比
              <div class="form-box">
                <Upload
                  :sheetsName="'YTDT_分货占比_31'"
                  @on-success="updatefhzb31"
                  @updateTimeChange="updatefhzb311"
                  :logTableName="'YTDT_Alloc_RegionRate_31'"
                  :logObj="'ytdtfhzb31'" >
                </Upload>
              </div>
            </h2>
            <div class="addgrid addgrid2">
              <v-table
                :sheetsName="'YTDT_分货占比_31'"
                :tableName="'YTDT_Alloc_RegionRate_31'"
                :source="'FF_IDSS_Data_YT'"
                :fileName="'YTDT_分货占比_31'"
                ref="ytdtfhzb31">
              </v-table>
            </div>
          </div>
        ...
        <el-tab-pane :label="menuArr[2]" class="alloItem">
          <div class="block"> 
            <h2>YTDT 31 参数校验
              ...
              <div class="form-box">
                <updata
                  @on-success="updateytdtsubmit31"
                  :text='"提交"'
                  :menuId="'804'"
                  :menuName="'分货参数_参数校验31'"
                  :logTableName="'YTDT_Alloc_Para_Check_31'"
                  :proIndex="'102'" >
                </updata>
              </div>
            ...
            <div class="addgrid addgrid3">
              <v-table
                :sheetsName="'YTDT_参数校验_31'"
                :tableName="'YTDT_Alloc_Para_Check_31'"
                :source="'FF_IDSS_Data_YT'"
                :fileName="'YTDT_参数校验_31'"
                ref="ytdtcheck31">
              </v-table>
            </div>                         
          </div>
...
export default {
  ...
  methods: {
    download1 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 0,
          DOWNLOAD_BAT_PATH: 'YTDT_分货参数校验_31_Download'
        },
        done (data) {
          let $a = $('<a></a>')
    ...
    sendemail1 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 1,
          SENDMAIL_BAT_PATH: 'YTDT_分货参数校验_31'
        },
        done (data) {
          alert(data)
    ...
    updateFhlx31 () {
      this.$refs.ytdtfhlx31.updateTabFun()
    },
    updatefhzb31 () {
      this.$refs.ytdtfhzb31.updateTabFun()
    },
    updateytdtcheck31 () {
      this.$refs.ytdtcheck31.updateTabFun()
    },
    updateytdtsubmit31 () {
      this.$refs.ytdtcheck31.updateTabFun()
    },
    updateFhlx311 () {
      this.$refs.ytdtfhlx31.updateTimeChange()
    },
    updatefhzb311 () {
      this.$refs.ytdtfhzb31.updateTimeChange()
    }
  }
}
</script>
