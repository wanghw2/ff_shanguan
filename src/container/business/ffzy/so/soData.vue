  ...
  <div class="formBox">
    <el-form ref="form" class="formBoxContent" :model="form" label-width="80px" size="mini">
      <div class="item2">
        ...
        <el-form-item label="订单类型" prop="orderType" class="item21" >
      ...
      <div class="item2">
        ...
        <el-form-item label="订单原因" prop="orderReason" class="item21" >
      ...
      <div class="box">
        ...
        <div class="item2">
          ...
          <el-form-item label="确认编码" prop="confirmPn"  class="item21">
            <el-input v-model="form.confirmPn" ></el-input>
          </el-form-item>
          ...
          <el-form-item label="大区"  prop="large" class="item25" >
        ...
        <div class="item2">
          ...
          <el-form-item label="确认描述" prop="confirmDesc" class="item21">
            <el-input v-model="form.confirmDesc" ></el-input>
          </el-form-item>
          ...
          <el-form-item label="省份" prop="shipProCode" class="item25" >
        ...
        <div class="item2">
          ...
          <el-form-item label="来单工厂" prop="tcFactory" class="item21" >
          ...
          <el-form-item label="发货工厂" prop="fhFactory" class="item25" >
      ...
      <el-form-item class="item3">
        ...
        <el-dropdown class="bianshi" size="mini" split-button type="primary">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-for="(item,index) in VariableList" :key="index" command="a"><span @click="transVariant(item.vid,item.variantValue)">{{item.variantName}}</span></el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
        <el-button type="primary" class="bianshi" @click.prevent="details">编辑变式</el-button>
      </el-form-item>
    </el-form>
  </div>
  <div class="lableDate">
  ...
  methods: {
      ...
      vm.columnsList.unshift(
        {command: [{
          ...
          click: function (e) {
            ...
            vm.$router.push(`/cnb/so/soDataShow`)
          }
        }],
          width: '90px'
        }
    ...
    getQueryVariant () { // 查询当前业务下所有变式
      ...
      vm.$ajax({
        ...
        params: {
          ...
          let systemList = [].concat(data).filter((item) => {
    ...
    valieForm () { // params参数
      ...
      if (this.form.confirmPn !== '') {
        params['confirmPn'] = this.form.confirmPn.replace(/ /g, ',')
      }
      if (this.form.matCode !== '') {
        params['matCode'] = this.form.matCode.replace(/ /g, ',')
      }
      if (this.form.matDesc !== '') {
        params['matDesc'] = this.form.matDesc.replace(/ /g, ',')
      }
      if (this.form.bpCode !== '') {
        params['bpCode'] = this.form.bpCode.replace(/ /g, ',')
      }
      if (this.form.bpName !== '') {
        params['bpName'] = this.form.bpName.replace(/ /g, ',')
      }
      if (this.form.confirmDesc !== '') {
        params['confirmDesc'] = this.form.confirmDesc.replace(/ /g, ',')
      }
      ...
      if (this.form.keyModel.join() !== '') {
        params['keyModel'] = this.form.keyModel.join()
      }
      if (this.form.proSub.join() !== '') {
        params['proSub'] = this.form.proSub.join()
      }
      if (this.form.orderState.join() !== '') {
        params['orderState'] = this.form.orderState.join()
      }
      if (this.form.channel.join() !== '') {
        params['channel'] = this.form.channel.join()
      }
      if (this.form.orderReason.join() !== '') {
        params['orderReason'] = this.form.orderReason.join()
      }
      if (this.form.orderType.join() !== '') {
        params['orderType'] = this.form.orderType.join()
      }
      if (this.form.matType.join() !== '') {
        params['matType'] = this.form.matType.join()
      }
      if (this.form.division.join() !== '') {
        params['division'] = this.form.division.join()
      }
      if (this.form.proSeries.join() !== '') {
        params['proSeries'] = this.form.proSeries.join()
      }
      if (this.form.tcFactory.join() !== '') {
        params['tcFactory'] = this.form.tcFactory.join()
      }
      if (this.form.fhFactory.join() !== '') {
        params['fhFactory'] = this.form.fhFactory.join()
      }
      if (this.form.cusSegment1.join() !== '') {
        params['cusSegment1'] = this.form.cusSegment1.join()
      }
      if (this.form.cusSegment4.join() !== '') {
        params['cusSegment4'] = this.form.cusSegment4.join()
      }
      if (this.form.shipProCode.join() !== '') {
        params['shipProCode'] = this.form.shipProCode.join()
      }
      if (this.form.large.join() !== '') {
        params['large'] = this.form.large.join()
      }
