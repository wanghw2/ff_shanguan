    ...
    <div class="content">
      <el-tabs :tab-position="tabPosition" style="height: auto;">
        <el-tab-pane :label="menuArr[0]" class="tarItem">
          <div class="block">
            <h2>ConDT 备货计划&emsp;&emsp;单位：k
              <div class="form-box">
                <upload
                  :sheetsName="'ConDT_备货计划'"
                  @on-success="updatePOtarget"
                  @updateTimeChange="updatePOtarget1"
                  :logTableName='"ConDT_PO_Target_Plan"'
                  :logObj="'gridPOtarget'">
                </upload>
              </div>
            </h2>
            <div class="addgrid">
              <v-table
                :sheetsName="'ConDT_备货计划'"
                :tableName='"ConDT_PO_Target_Plan"'
        ...
        <el-tab-pane :label="menuArr[1]" class="tarItem">
          <div class="block">
             ...
             <a :class="['small', 'emailBtn', {'disabled': disabledemail}]" @click="sendemail1" href="#">发送邮件</a>
              ...
              <div class="form-box">
                <updata
                  ...
                  :logTableName="'ConDT_Supply_Cadence_TTL'"
                  :proIndex="'1'" >
                </updata>
              </div>            
            </h2>
            <div class="addgrid">
              <v-table
                :sheetsName="'ConDT_Supply_Cadence'"
                :tableName='"ConDT_Supply_Cadence_TTL"'
                :source="'FF_IDSS_Data_CON'"
                :fileName="'ConDT_Supply_Cadence'"
                ref="gridspcadence">
              </v-table>
            </div>
          </div>
    ...
    download1 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 0,
          DOWNLOAD_BAT_PATH: 'ConDT_Supply_Cadence_Download'
        },
        done (data) {
          let $a = $('<a></a>')
    ...
    sendemail1 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 1,
          SENDMAIL_BAT_PATH: 'ConDT_Supply_Cadence'
        },
        done (data) {
          alert(data)
