      ...
      <el-tabs :tab-position="tabPosition" style="height: auto;">
        <el-tab-pane :label="menuArr[0]" class="tarItem">
          <div class="block">
            ...
            <div class="addgrid">
              <v-table
                :sheetsName="'ConDT_PSD当日_异常监控'"
                :tableName="'ConDT_DF_PSD_Today_Tracking'"
                :source="'FF_IDSS_Data_CON'"
                :fileName="'ConDT_PSD当日_异常监控'"
                ref="cdtDFpsdtoday">
              </v-table>
            </div>
          </div>
        ...
        <el-tab-pane :label="menuArr[1]" class="tarItem">
          <div class="block">
            <h2>改单分析
              <div class="form-box">
                <upload
                  @on-success="cdtCAweekRatio"
                  @updateTimeChange="cdtCAweekRatio1"
                  :sheetsName="'ConDT_5S节奏'"
                  :logTableName="'ConDT_CA_WeekRatio'"
                  :logObj="'cdtweekRatio'" >
                </upload>
              </div>
            </h2>
        ...
        <el-tab-pane :label="menuArr[3]" class="tarItem">
          <div class="block">
            <h2>ConDT FPSD手工订单
              <div class="form-box">
                <upload
                  :sheetsName="'ConDT_FPSD手工订单'"
                  @on-success="updatefpsdsg"
                  @updateTimeChange="updatefpsdsg1"
                  :logTableName='"ConDT_FPSD_Manage"'
                  :logObj="'gridfpsdsg'">
                </upload>
              </div>
            </h2>
            <div class="addgrid">
              <v-table
                :sheetsName="'ConDT_FPSD手工订单'"
                :tableName="'ConDT_FPSD_Manage'"
                :source="'FF_IDSS_Data_CON'"
                :fileName="'ConDT_FPSD手工订单'"
                ref="gridfpsdsg">
              </v-table>
            </div>
          </div>
        ...
        <el-tab-pane :label="menuArr[4]" class="tarItem">
          <div class="block">
            <h2>ConDT FPSD手工订单 汇总</h2>
            <div class="addgrid">
              <v-table
                :sheetsName="'ConDT_FPSD手工订单_汇总'"
                :tableName="'ConDT_FPSD_Manage_TTL'"
                :source="'FF_IDSS_Data_CON'"
                :fileName="'ConDT_FPSD手工订单_汇总'"
                ref="gridfpsdsgttl">
              </v-table>
            </div>
          </div>
...
export default {
  data () {
    return {
      serverUrl: domainUrl,
      tabPosition: 'left',
      menuArr: ['PSD当日异常', '改单分析', '撤单分析', 'FPSD手工订单', 'FPSD手工订单 汇总']
    }
  },
  components: {
    Upload,
  ...
  methods: {
    ...
    // cdtTogoso () { // 上传后自动刷新表格数据
    //   this.$refs.cdtTogoso.updateTabFun()
    // },
    // cdtTogoso1 () { // 上传后自动刷新表格数据
    //   this.$refs.cdtTogoso.updateTimeChange()
    // },
    // cdtDFpsdtoday () { // 上传后自动刷新表格数据
    //   this.$refs.cdtDFpsdtoday.updateTabFun()
    // },
    // cdtDFpsdtoday1 () { // 上传后自动刷新表格数据
    //   this.$refs.cdtDFpsdtoday.updateTimeChange()
    // },
    updatefpsdsg () {
