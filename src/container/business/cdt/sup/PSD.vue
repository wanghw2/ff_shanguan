        ...
        <el-tab-pane :label="menuArr[0]" class="supitem">
          <div class="block">
            <h2>LBG DT PSD
              ...
              <a :class="['small', 'emailBtn', {'disabled': disabledemail}]" @click="sendemail0" href="#">LBG邮件</a>
              <div class="form-box">
                <upload
                  :sheetsName="'LBG_DT_PSD'"
                  @on-success="updatePSDReport"
                  @updateTimeChange="updatePSDReport1"
                  :logTableName="'LBG_DT_PSD'"
                  :logObj="'gridlbgdtpsd'">
                </upload>
              </div>
            </h2>
            ...
            <h2>ConDT PSD
              ...
              <a :class="['small', 'emailBtn', {'disabled': disabledemail}]" @click="sendemail1" href="#">ConDT邮件</a>
              <a :class="['small', {'disabled': disableddownload}]" class="small dowDTtn" @click="download1" href="#">下载</a>
            </h2>
            <div class="addgrid addgrid2">
              <v-table
                :sheetsName="'ConDT_PSD_Detail'"
                :tableName="'ConDT_PSD_Detail'"
                :source="'FF_IDSS_Data_CON'"
                :fileName="'ConDT_PSD_Detail'"
                ref="gridconDTpsd">
              </v-table>
            </div>
            <h2>ConDT 后续可供
              ...
              <a :class="['small', {'disabled': disableddownload}]" class="small dowDTtn" @click="download2" href="#">下载</a>
              <div class="form-box">
                <updata
                  @on-success="updateTogosupply"
                  :menuId="'310'"
                  :menuName="'PSD管理_PSD上传'"
                  :logTableName="'ConDT_PSD_Update'"
                  :proIndex="'1'" >
                </updata>
              </div>            
            </h2>
            <div class="addgrid addgrid3">
              <v-table
                :sheetsName="'ConDT_PSD_Update'"
                :tableName='"ConDT_PSD_Update"'
                :source="'FF_IDSS_Data_CON'"
                :fileName="'ConDT_PSD_Update'"
                ref="gridTogosupply">
              </v-table>
            </div>
          </div>
          ...
          <div class="block">
            ...
            <div class="addgrid addgrid7">
              <v-table
                ...
                :tableName='"ConDT_GEO_Order_Candence"'
                :source="'FF_IDSS_Data_CON'"
            ...
            <div class="addgrid addgrid8">
              <v-table
                :sheetsName="'Input分析'"
                :tableName='"ConDT_Input_Analysis"'
                :source="'FF_IDSS_Data_CON'"
                :fileName="'Input分析'">
              </v-table>
            </div>
...
export default {
  ...
  methods: {
    sendemail0 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 1,
          SENDMAIL_BAT_PATH: 'LBG_DT_PSD_Report'
        },
        done (data) {
          alert(data)
    ...
    download1 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 0,
          DOWNLOAD_BAT_PATH: 'ConDT_PSD_Report_Download'
        },
        done (data) {
          let $a = $('<a></a>')
    ...
    sendemail1 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 1,
          SENDMAIL_BAT_PATH: 'ConDT_PSD_Report'
        },
        done (data) {
          alert(data)
          vm.disabledemail = false
    ...
    download2 () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 0,
          DOWNLOAD_BAT_PATH: 'ConDT_后续可供_Download'
        },
        done (data) {
          let $a = $('<a></a>')
          $a.attr('href', data)[0].click()
    ...
    updatePSDReport () {
      this.$refs.gridlbgdtpsd.initTabTable()
      this.$refs.gridconDTpsd.initTabTable()
    },
    updatePSDReport1 () {
      this.$refs.gridlbgdtpsd.updateTimeChange()
      this.$refs.gridconDTpsd.updateTimeChange()
    },
    gagueBoarddata () {
      let vm = this
      vm.$ajax({
    ...
    // updatePSDReport () {
    //   this.$refs.cDTPSDReport.updateTabFun()
    // },
    // updatePSDReport1 () {
    //   this.$refs.cDTPSDReport.updateTimeChange()
    // },
    // drawStack () {
    //   let vm = this
    //   vm.$ajax({
