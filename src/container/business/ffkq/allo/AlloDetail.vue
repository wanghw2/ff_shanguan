        ...
        <el-tab-pane :label="menuArr[0]" class="pdtItem">
          <div class="block">
            <h2>KQ 四六级进度监控表
              ...
              <div class="form-box">
                <updata
                  @on-success="updatebtspn"
                  :menuId="'703'"
                  :menuName="'分货详情_进度监控'"
                  :logTableName="'KQ_DailyReport_BTS_PN'"
                  :proIndex="'1'" >
                </updata>
              </div>
            </h2>
            <div class="addgrid">
              <v-table
                :sheetsName="'KQ_四六级进度监控表'"
                :tableName="'KQ_DailyReport_BTS_PN'"
                :source="'FF_IDSS_Data_REL'"
                :fileName="'KQ_四六级进度监控表'"
                ref="gridbtspn">
              </v-table>
            </div>
          </div>
...
export default {
  ...
  methods: {
    download () {
      ...
      vm.$ajax({
        ...
        params: {
          type: 1,
          ismail: 0,
          DOWNLOAD_BAT_PATH: 'KQ_四六级进度监控表_Download'
        },
        done (data) {
          let $a = $('<a></a>')
    ...
    updatebtspn () {
      this.$refs.gridbtspn.updateTabFun()
    }
  }
}
</script>
